...
```bash
# Newer llama.cpp requires explicit value (on/off/auto)
./llama-server -m model.gguf -ngl 99 --flash-attn on

# Older llama.cpp accepted standalone flag; if you get:
#   "error: unknown value for --flash-attn: '--no-webui'"
# it means your version requires --flash-attn on|off|auto
./llama-cli -m model.gguf -ngl 35 --flash-attn on -p "Hello"
```
...
**Symptoms**:
- Loading an MTP model logs:\n  `W srv    load_model: speculative decoding will use checkpoints`\n  `W common_speculative_init: no implementations specified for speculative decoding`
- API returns `content: ""` with garbled `reasoning_content` text
- Prompt processing works but generated output is nonsense

**Cause**: MTP models trigger speculative decoding. Without a draft model, output can be garbled.

**Fixes**:
1. **Disable speculative decoding**: `./llama-server -m model.gguf --no-speculative`
2. **Use non-MTP variant** of the GGUF if available (avoids the issue entirely)

### Thinking / Reasoning Model Output

**Symptoms**:
- Server logs `chat template, thinking = 1`
- API returns empty `content` with garbled `reasoning_content`
- Model works fine in llama-cli but broken in server mode

**Cause**: llama-server's auto-detected thinking template (`<think>...</think>`) can garble output when paired with certain tokenizers or speculative decoding.

**Fixes**:
1. **Disable speculative decoding**: `./llama-server -m model.gguf --no-speculative`
2. **Test with llama-cli first**: `./llama-cli -m model.gguf -p "Hello" -n 50 --temp 0`
3. **Try a different server version** — newer builds handle thinking models better
