...
### 2b. 图文笔记文章（正文全为图片）
部分公众号文章（医管笔记等）正文是图片而非文字。处理：
- 提取所有 `<img data-src>` URL，curl 下载到 /tmp
- 用本地 LM Studio 多模态模型 OCR（`http://127.0.0.1:1234/v1/chat/completions`，qwen3.8-27b@q8_0，base64 内联图片）
- 每张图约 1-2 分钟，10 张图需后台脚本 + 轮询（execute_code 300s 会超时，用 terminal background 跑）
- OCR 提示词："请完整提取这张图片中的所有文字内容，保持原结构和层次，不要遗漏、不要总结、不要添加解释。只要原文。"
- 偶发单张 timed out，单独重试即可
