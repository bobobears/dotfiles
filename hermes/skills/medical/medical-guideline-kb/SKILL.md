...
├── medical/疾病知识/                  # 疾病类知识：科普解读、筛查工具（IOF/OSTA等）、患者教育材料
...
### 2g. 政府官网文章（WAF 反爬，如 nhc.gov.cn）
国家卫健委官网（nhc.gov.cn）用 icloudwaf：**curl 返回 HTTP 412**（JS 挑战页），**headless 浏览器返回 title="🐴"、HTML 仅57字节的空页**；`web_extract` 报 Failed to fetch；archive.org 国内多超时。绕行路径：

1. **用 `web_search`（tavily）读取栏目内容** — tavily 能读到 nhc 页面文本。搜「国家卫健委 <司局> <栏目> <年月> 通知」即可拿到**栏目列表页的完整条目（标题+日期）**，据此锁定目标文章（不必知道原 URL 对应哪篇）
2. **从权威转载源取全文** — 国家级文件常被学会（如中国老年保健医学研究会 caogr.org）、地方政府站、行业媒体全文转载。抓取后**必核对**：发文字号、印发机关、日期、章节数/结尾是否完整
3. **入库时在"文档说明"里写明**：原文链接 + 实际抓取源 + WAF 原因，避免日后误判为官网原文
4. **官方原文受验证码保护时，请用户下载 PDF 后入库**——官方站（如 openstd 强制性国标）的"下载"按钮设图形验证码，**不要绕验证机制**；把链接给用户（他输验证码只需几秒），请他下载后放入 `~/下载/知识/`，再用 2a 的 PDF 流程提取。验证码拦截不是失败，是分工。
5. **拿到正式文本后，回头核对之前基于自媒体的条目**：自媒体披露的具体数字常有错（实例：某公众号称养老护理员配比 1:4/1:5/1:12/1:20，GB 48013—2026 原文 9.2.7 为 1:5/1:10/1:15），用 patch 把旧条目的"无法证实"警告改写成逐条核对表

已入库案例（政府站 WAF）：《医养结合机构管理指南（试行）》（国卫办老龄发〔2020〕15号，nhc 栏目 lljks/c100158；经 caogr.org 转载取全文，7 章完整）；《养老机构基本规范》GB 48013—2026（openstd 预览仅低清缩略+下载需验证码 → 请用户下载 PDF → pdftotext -layout 提取 15 页全文）
