...
验证 HTML 渲染结果时，测试脚本本身经常产生假阳性：

1. **`display:none` 检测窗口太大** — `m.start()-200` 会包含邻近按钮的 `display:none`。将检测范围缩小到 `[m.start()-5:m.end()]`
2. **emoji vs 纯文本** — 不同模板可能用 `📄 合同管理` 和 `合同管理` 两种格式，验证脚本必须同时覆盖
3. **CSS class 定义干扰** — CSS `<style>` 中的 class 名被误认为 HTML 元素
4. **Button class 重复** — 导航按钮和行级操作按钮共用 `btn-outline btn-sm` 时，正则匹配出大量无关结果

> 完整实例见 `references/html-verification-pitfalls.md`
